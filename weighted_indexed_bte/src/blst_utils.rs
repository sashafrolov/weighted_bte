...
use blst::{
    blst_p1, blst_p1s_to_affine, blst_p2, blst_p2_affine, blst_p2s_mult_pippenger,
    blst_p2s_mult_pippenger_scratch_sizeof, blst_p2s_to_affine,
};
#[cfg(test)]
use blst::{blst_p1_affine, blst_p1s_mult_pippenger, blst_p1s_mult_pippenger_scratch_sizeof};
...
/// Perform a single-threaded raw-BLST MSM over affine G1 points.
///
/// Retained in tests to ensure the raw G1 path inherited from BTX agrees with
/// `blstrs`; this indexed construction needs only one scalar multiplication
/// for each online partial decryption.
#[cfg(test)]
...
/// Perform a BLST Pippenger MSM directly from affine G2 points.
///
/// `blstrs::G2Projective::multi_exp` accepts projective inputs and first batch
...
#[cfg(test)]
pub(crate) fn g2_multi_exp_affine(points: &[G2Affine], scalars: &[Scalar]) -> G2Projective {
    assert_eq!(points.len(), scalars.len());
    g2_multi_exp_affine_bytes(points, &scalars_to_le_bytes(scalars))
}
...
pub(crate) fn g2_multi_exp_affine_bytes(points: &[G2Affine], scalar_bytes: &[u8]) -> G2Projective {
    ...
    // G2Affine is a transparent wrapper around blst_p2_affine. BLST's block
    ...
    g2_multi_exp_affine_raw(&point_blocks, points.len(), scalar_bytes)
}
...
#[cfg(test)]
pub(crate) fn g2_multi_exp_affine_indexed(
    points: &[G2Affine],
    indices: &[usize],
    scalar_bytes: &[u8],
) -> G2Projective {
    ...
    point_blocks.extend(indices.iter().map(|index| {
        points
            ...
            .as_ref() as *const blst_p2_affine
    }));
    point_blocks.push(std::ptr::null());
    g2_multi_exp_affine_raw(&point_blocks, indices.len(), scalar_bytes)
}

fn g2_multi_exp_affine_raw(
    point_blocks: &[*const blst_p2_affine],
    point_count: usize,
    scalar_bytes: &[u8],
) -> G2Projective {
    ...
    let scalar_blocks: [*const u8; 2] = [scalar_bytes.as_ptr(), std::ptr::null()];
    let scratch_size = unsafe { blst_p2s_mult_pippenger_scratch_sizeof(point_count) };
    let word_size = std::mem::size_of::<u64>();
    let mut scratch = vec![0u64; scratch_size.div_ceil(word_size)];
...
mod tests {
    ...
    fn affine_g2_msm_matches_blstrs_pippenger() {
        ...
        assert_eq!(g2_multi_exp_affine(&affine, &scalars), expected);
        assert_eq!(g2_multi_exp_affine(&[], &[]), G2Projective::identity());
    }
    ...
    fn affine_g1_msm_matches_blstrs_pippenger() {
        ...
        assert_eq!(
            g1_multi_exp_affine_bytes(&affine, &scalars_to_le_bytes(&scalars)),
            expected
        );
        assert_eq!(
            g1_multi_exp_affine_bytes(&[], &[]),
            G1Projective::identity()
        );
    }

    #[test]
    fn indexed_affine_g2_msm_matches_selected_points() {
        let points = (0..80)
            .map(|index| {
                (G2Projective::generator() * Scalar::from((3 * index + 1) as u64)).to_affine()
            })
            .collect::<Vec<_>>();
        ...
        let selected_projective = indices
            .iter()
            .map(|index| G2Projective::from(points[*index]))
            .collect::<Vec<_>>();
        let expected = G2Projective::multi_exp(&selected_projective, &scalars);

        assert_eq!(
            g2_multi_exp_affine_indexed(&points, &indices, &scalars_to_le_bytes(&scalars)),
            expected
        );
        assert_eq!(
            g2_multi_exp_affine_indexed(&points, &[], &[]),
            G2Projective::identity()
        );
    }
