...
#[cfg(test)]
use blst::{blst_fp6, blst_miller_loop_lines, blst_precompute_lines};
...
///
/// Swapped weighted BTX keeps its static kernel and opening keys in G1.
/// The ciphertexts and party shares are the dynamic G2 operands. BLST can
/// consume two contiguous affine arrays directly, sharing the Miller loop
/// across all terms while keeping only one Fp12 accumulator.
