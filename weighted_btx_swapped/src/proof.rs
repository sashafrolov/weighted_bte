...
const TRANSCRIPT_DOMAIN: &[u8] = b"WEIGHTED-BTX-SWAPPED-SCHNORR-BLS12381-v1";
...
impl SchnorrProof {
    pub fn create<R: rand_core::RngCore + rand_core::CryptoRng>(
        randomness: Scalar,
        ciphertext_first: G2Projective,
        ciphertext_second: &Gt,
        setup_id: [u8; 32],
        rng: &mut R,
    ) -> Self {
        let nonce = Scalar::random(rng);
        let commitment = G2Projective::generator() * nonce;
    ...
    pub fn verify(
        ...
        G2Projective::generator() * self.response == self.commitment + ciphertext_first * challenge
    }
}
...
fn challenge_scalar(
    ...
    let generator_bytes = G2Affine::from(G2Projective::generator()).to_compressed();
    let ciphertext_bytes = G2Affine::from(ciphertext_first).to_compressed();
    let commitment_bytes = G2Affine::from(commitment).to_compressed();
...
mod tests {
    ...
    fn valid_proof_verifies_and_tampering_fails() {
        ...
        let ct1 = G2Projective::generator() * randomness;
        ...
        assert!(!proof.verify(ct1 + G2Projective::generator(), &ct2, setup_id));
