...
const BATCH_DIGEST_DOMAIN: &[u8] = b"WEIGHTED-BTX-ORDERED-BATCH-v1";
const DECRYPTION_CONTEXT_DOMAIN: &[u8] = b"WEIGHTED-BTX-DECRYPTION-CONTEXT-v1";
...
/// Committee-dependent G2 work reused for batches with the same size and
...
pub fn partial_decrypt(
    ...
    Ok(DecryptionShare {
        ...
        sigma: g1_multi_exp_affine_bytes(&batch.first_affine, &scalar_bytes),
...
fn verify_decryption_shares_batched(
    ...
    let mut share_affine = vec![G1Affine::default(); share_points.len()];
    batch_normalize_g1(&share_points, &mut share_affine);
    ...
    let mut left = valid_slots
        ...
        .map(|slot| batch.first_affine[*slot])
        .collect::<Vec<_>>();
    left.push(G1Affine::from(-aggregate_sigma));
...
pub fn prepare_decryption(
    ...
    // the reuse responsible for the `(B-1) MSM_2(W_T)` row in Table 2.
    let positive = (1..accepted.batch_size)
        ...
        .map(|distance| {
            ...
            Ok((
                distance,
                g2_multi_exp_affine_indexed(power, &selected_virtual_indices, &coefficient_bytes),
            ))
        })
        .collect::<Result<Vec<_>>>()?;
    ...
    let mut opening_keys = vec![G2Affine::default(); opening_projective.len()];
...
pub fn precompute_batch(
    ...
    let mut transformed_ciphertexts = vec![G1Projective::identity(); precomputation.transform_size];
    ...
    let mut transformed_affine = vec![G1Affine::default(); precomputation.transform_size];
...
pub fn open_batch(
    ...
    let share_projective = accepted
        ...
        .collect::<Vec<_>>();
    let mut share_affine = vec![G1Affine::default(); share_projective.len()];
...
mod tests {
    ...
    use super::*;
    use crate::{blst_utils::g2_multi_exp_affine, encrypt, keygen};
                ...
                (0..batch.batch_size)
                    ...
                    .map(|input_slot| {
                        ...
                        let bases = selected
                            ...
                            .collect::<Vec<_>>();
                        let aggregate = g2_multi_exp_affine(&bases, &coefficients);
                        blstrs::Bls12::pairing(
                            &G1Affine::from(batch.first_projective[input_slot]),
    ...
    fn malformed_share_is_blamed_and_honest_authorized_remainder_opens() {
        ...
        let mut shares = [1usize, 2, 3]
            ...
            .map(|index| partial_decrypt(&material.party_keys[*index], &batch).unwrap())
            .collect::<Vec<_>>();
        shares[1].sigma += G1Projective::generator();
