...
const SETUP_ID_DOMAIN: &[u8] = b"WEIGHTED-BTX-SETUP-ID-v1";
...
///
/// Every large G2 array is exponent-major. One contiguous block therefore
...
pub struct PublicDecryptionKey {
    ...
    /// `[(q_j)^i]_2`, indexed as `(i - 1) * party_count + j`.
    verification_keys: Box<[G2Affine]>,
    /// `[Z(omega) q_owner(omega)^(-i)]_2`, in `i`-major W-sized blocks.
    negative_material: Box<[G2Affine]>,
    /// `[Z(omega) q_owner(omega)^i]_2`, for `i = 1` through `L - 1`, in W-sized blocks.
    positive_material: Box<[G2Affine]>,
}
...
pub fn keygen_with_rng<R: rand_core::RngCore + rand_core::CryptoRng>(
    ...
    for power_index in 0..config.max_batch_size {
        ...
        append_g2_block(&mut negative_material, &scalar_block);
    }
    ...
    for power_index in 0..config.max_batch_size.saturating_sub(1) {
        ...
        append_g2_block(&mut positive_material, &scalar_block);
    }
...
impl PublicDecryptionKey {
    ...
    pub(crate) fn verification_key(&self, party_index: usize, power: usize) -> Result<G2Affine> {
        if party_index >= self.party_count() {
            return Err(Error::InvalidPartyIndex(party_index));
        }
    ...
    pub(crate) fn verification_power(&self, power: usize) -> Result<&[G2Affine]> {
        validate_power(power, self.max_batch_size)?;
        let start = (power - 1) * self.party_count();
        Ok(&self.verification_keys[start..start + self.party_count()])
    }

    pub(crate) fn negative_power(&self, power: usize) -> Result<&[G2Affine]> {
        validate_power(power, self.max_batch_size)?;
        let start = (power - 1) * self.total_weight;
        Ok(&self.negative_material[start..start + self.total_weight])
    }

    pub(crate) fn positive_power(&self, power: usize) -> Result<&[G2Affine]> {
...
fn append_g2_block(output: &mut Vec<G2Affine>, scalars: &[Scalar]) {
    ...
    output.resize(old_len + projective.len(), G2Affine::default());
    batch_normalize_g2(&projective, &mut output[old_len..]);
}
...
fn setup_identifier(
    ...
    threshold_weight: usize,
    party_weights: &[usize],
    first_verification_power: &[G2Affine],
    first_negative_power: &[G2Affine],
) -> [u8; 32] {
            ...
            for exponent in 1..=4 {
                assert_eq!(
                    dk.verification_key(party_key.party_index, exponent)
                        .unwrap(),
                    (G2Projective::generator() * power).to_affine()
                );
    ...
    fn negative_and_positive_material_use_the_owning_party_power() {
        ...
        for party_key in &material.party_keys {
            ...
            let baseline = dk.negative_power(1).unwrap()[range.clone()]
                .iter()
                .map(|point| G2Projective::from(*point) * q)
                .collect::<Vec<_>>();
            ...
            for power in 1..=3 {
                let recovered = dk.negative_power(power).unwrap()[range.clone()]
                    .iter()
                    .map(|point| G2Projective::from(*point) * q_power)
                    .collect::<Vec<_>>();
            ...
            for power in 1..=2 {
                let recovered = dk.positive_power(power).unwrap()[range.clone()]
                    .iter()
                    .map(|point| G2Projective::from(*point) * inverse_power)
                    .collect::<Vec<_>>();
                assert_eq!(recovered, baseline);
        ...
        let bases = selected
            ...
            .map(|virtual_index| {
                let party_index = material
                    ...
                    .unwrap();
                G2Projective::from(dk.negative_power(1).unwrap()[*virtual_index])
                    * material.party_keys[party_index].secret_scalar()
            })
            .collect::<Vec<_>>();
        let reconstructed = G2Projective::multi_exp(&bases, &coefficients).to_affine();

        assert_eq!(
            pairing(&G1Affine::generator(), &reconstructed),
